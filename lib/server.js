...
function registerPlugin(){
  return new Promise(function(resolve, reject){
    ...
    server.register(CookieAuth, (err)=>{
      ...
      //by server.cache segment/name as 'sessions', expire in a day or after quit the browser
      ...
      server.auth.strategy('base', 'cookie', {
        ...
        cookie: 'cookieCookie', // Cookie name
        ...
        validateFunc: function(req, session, callback){
          ...
          */
          //cache.get will take tin session.id, and match with it value
          //if I havn't logged in then session.sid is undefined
          //and the value should be undefined also.
          //then if(!value) undefine so return callback with false value
          //the false value kick off something to redirect to /login page.
          //I am going to find out where it is
          //if I login with gphan username (the const sid = usrname in login page)
          //then session.sid should be gphan
          //and cache.set(sid,{credentials:credentials}) in login page will yield
          //value as my username and password; sid is the key, password is the value
          //the callback function somehow just redirect the page to the right page
          ...
          cache.get(session.sid, (err, value) => {
              ...
              //or else if the value is not empty then
              //return true to to the callback function
              //and add the credentials into the the session
